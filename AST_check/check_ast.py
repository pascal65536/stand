...
class FeatureExtractor:
    ...
    def _visit_Call(self, node):
        ...
        if func.get('_type') == 'Attribute':
            ...
            if attr == 'system' and val.get('_type') == 'Name' and val.get('id') == 'os':
                self.features['call_os_system'] += 1
                self.features['dynamic_code_exec'] += 1
            elif attr == 'chmod' and val.get('_type') == 'Name' and val.get('id') == 'os':
                self.features['call_os_chmod'] += 1
            elif attr == 'setuid' and val.get('_type') == 'Name' and val.get('id') == 'os':
                self.features['call_os_setuid'] += 1
            elif attr == 'remove' and val.get('_type') == 'Name' and val.get('id') == 'os':
                self.features['call_os_remove'] += 1
            elif attr == 'rename' and val.get('_type') == 'Name' and val.get('id') == 'os':
                self.features['call_os_rename'] += 1

            # subprocess.Popen
            elif attr == 'Popen' and val.get('_type') == 'Name' and val.get('id') == 'subprocess':
                self.features['call_subprocess_popen'] += 1
                self.features['dynamic_code_exec'] += 1

            # ctypes.cdll.LoadLibrary, ctypes.cast
            elif attr == 'LoadLibrary' and val.get('_type') == 'Attribute' and \
                 val.get('value', {}).get('_type') == 'Name' and val.get('value', {}).get('id') == 'ctypes' and val.get('attr') == 'cdll':
                self.features['call_ctypes_loadlibrary'] += 1
                self.features['dynamic_code_exec'] += 1
            elif attr == 'cast' and val.get('_type') == 'Name' and val.get('id') == 'ctypes':
                self.features['call_ctypes_cast'] += 1
                self.features['dynamic_code_exec'] += 1

            # shutil.rmtree
            elif attr == 'rmtree' and val.get('_type') == 'Name' and val.get('id') == 'shutil':
                self.features['call_shutil_rmtree'] += 1

            # socket.socket, socket.bind, socket.connect
            elif attr == 'socket' and val.get('_type') == 'Name' and val.get('id') == 'socket':
                self.features['use_socket_socket'] += 1
            elif attr == 'bind' and val.get('_type') == 'Name' and val.get('id') == 'socket':
                self.features['use_socket_bind'] += 1
            elif attr == 'connect' and val.get('_type') == 'Name' and val.get('id') == 'socket':
                self.features['use_socket_connect'] += 1

            # HTTP-запросы
            if attr in ('get', 'post', 'urlopen'):
                if val.get('_type') == 'Name' and val.get('id') in ('requests', 'urllib', 'urllib2', 'urllib_request', 'urllib.request'):
                    self.features['use_http_request'] += 1
                elif val.get('_type') == 'Attribute' and val.get('attr') in ('request',):
                    self.features['use_http_request'] += 1

            # Декодирование строк
            if (val.get('_type') == 'Name' and val.get('id') in ('base64', 'binascii')) or \
               (val.get('_type') == 'Attribute' and val.get('attr') in ('base64', 'binascii')):
                if attr in ('b64decode', 'a85decode', 'unhexlify', 'hexlify'):
                    self.features['string_decoding_calls'] += 1

            if (val.get('_type') == 'Name' and val.get('id') == 'zlib') and attr == 'decompress':
                self.features['string_zlib_decompress_calls'] += 1

            # Регулярные выражения
            if (val.get('_type') == 'Name' and val.get('id') == 're') and attr == 'compile':
                self.features['regex_compile_calls'] += 1
