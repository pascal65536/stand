EDUCATIONAL_RULES = [
    # Имена переменных
    {"target": "store_vars", "check": "name", "condition": "len(name) == 1 and name not in ['_', 'i', 'j', 'k']", "message": "Односимвольное имя переменной '{name}' (строки {lines})", "code": "EDU-VAR-001", "severity": "high"},
    {"target": "store_vars", "check": "name", "condition": "name in ['l', 'o', 'O']", "message": "Имя переменной '{name}' визуально похоже на цифру", "code": "EDU-VAR-002", "severity": "high"},
    {"target": "store_vars", "check": "name", "condition": "not name[0].isalpha() and name[0] != '_'", "message": "Имя переменной '{name}' должно начинаться с буквы или _", "code": "EDU-VAR-003", "severity": "medium"},
    
    # Импорты
    {"target": "imports", "check": "name", "condition": "'re' in name", "message": "Запрещённый модуль 're' (регулярные выражения) в учебном задании", "code": "EDU-IMP-001", "severity": "medium"},
    {"target": "imports", "check": "name", "condition": "name in ['os', 'sys', 'subprocess']", "message": "Системные модули запрещены в учебном задании: {name}", "code": "EDU-IMP-002", "severity": "high"},
    
    # Вызовы функций
    {"target": "function_calls", "check": "name", "condition": "name in ['eval', 'exec', 'compile']", "message": "Опасная функция '{name}' запрещена", "code": "EDU-CALL-001", "severity": "critical"},
    
    # Классы
    {"target": "class_names", "check": "name", "condition": "not name[0].isupper()", "message": "Класс '{name}' должен начинаться с заглавной буквы (PEP 8)", "code": "EDU-CLS-001", "severity": "medium"},
    
    # Структурные ограничения
    {"target": "genexp", "check": "absent", "condition": "True", "message": "Генераторные выражения запрещены в учебном задании", "code": "EDU-STRUCT-001", "severity": "high"},
    {"target": "lambda", "check": "absent", "condition": "True", "message": "Анонимные функции (lambda) запрещены", "code": "EDU-STRUCT-002", "severity": "medium"}
]
