...
class DjangoJSONVulnerabilityAnalyzer:
    ...
    def _visit_FunctionDef(self, node: Dict[str, Any]):
        ...
        for decorator in node.get('decorator_list', []):
            if decorator.get('_type') == 'Name' and decorator.get('id') == 'csrf_exempt':
                ...
                if self._function_modifies_state(node):
                    self.report(node, "CSRF_PROTECTION_DISABLED", f"Декоратор @csrf_exempt применен к функции '{node['name']}', которая изменяет состояние. Высокий риск CSRF-атаки.")
    ...
    def _visit_Call(self, node: Dict[str, Any]):
        ...
        if self._is_get_password_in_get(node):
            self.report(node, "SENSITIVE_DATA_IN_GET", "Чувствительные данные (пароль, токен) передаются через GET-параметр.")
    ...
    def _is_django_template_from_string(self, node: Dict[str, Any]) -> bool:
        ...
        if inner_func.get('_type') != 'Attribute' or inner_func.get('attr') != 'from_string':
            ...
            if value.get('_type') == 'Attribute':
                ...
                if attr_value.get('_type') == 'Subscript':
                    ...
                    if sub_value.get('_type') == 'Name' and sub_value.get('id') == 'engines':
                        slice_ = attr_value.get('slice', {})
                        if slice_.get('_type') == 'Constant' and slice_.get('value') == 'django':
                            return True
        return False
    ...
    def _check_xss_in_template_string(self, node: Dict[str, Any]):
        ...
        if len(args) > 0:
            ...
            if first_arg.get('_type') == 'JoinedStr':
                self.report(node, "XSS_VIA_TEMPLATE", "Обнаружена f-строка в шаблоне Django. Возможна XSS-уязвимость.")
            ...
            if arg.get('_type') == 'Call':
                ...
                if func.get('_type') == 'Attribute' and func.get('attr') == 'get':
                    ...
                    if value.get('_type') == 'Attribute' and value.get('attr') == 'GET':
                        ...
                        if request_value.get('_type') == 'Name' and request_value.get('id') == 'request':
                            self.report(node, "OPEN_REDIRECT", "Обнаружен Open Redirect: перенаправление на URL из request.GET.")
    ...
    def _check_access_control(self, node: Dict[str, Any]):
        ...
        self.report(node, "POTENTIAL_BROKEN_ACCESS_CONTROL", f"Обнаружена опасная операция '{attr}' без явной проверки прав доступа.")
                ...
                if request_value.get('_type') == 'Name' and request_value.get('id') == 'request':
                    ...
                    if len(args) > 0:
                        ...
                        if first_arg.get('_type') == 'Constant':
                            ...
                            sensitive_keywords = {'pass', 'pwd', 'secret', 'token', 'key'}
            ...
            def visit(self, n):
                if isinstance(n, dict):
                    if n.get('_type') == 'Call':
                        func = n.get('func', {})
                        if func.get('_type') == 'Attribute':
                            if func.get('attr') in ['save', 'delete', 'update', 'create']:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Использование: python custom_analyzer.py <путь_к_исходнику.py> <путь_к_ast.json>")
        print("Пример: python custom_analyzer.py ./insecure_app/views.py ./Pasted_Text_1758296001917.txt")
        sys.exit(1)
