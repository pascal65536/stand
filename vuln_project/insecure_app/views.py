...
def load_yaml_data(request):
    if request.method == "POST":
        data = yaml.load(
            request.body, Loader=yaml.FullLoader
        )  # <-- Уязвимость! CWE-502
        return HttpResponse(f"Loaded: {data}")
    return HttpResponse("Send POST data.")
...
def set_password_insecure(request):
    ...
    hashed = hashlib.md5(
        password.encode()
    ).hexdigest()  # <-- Уязвимость! CWE-327, CWE-916
...
def cache_user_data(request):
    ...
    cache.set(
        f"user_{username}", user, timeout=3600
    )  # <-- Уязвимость! CWE-524, CWE-312
...
def transfer_money(request):
    if request.method == "POST":
        ...
        return HttpResponse(
            f"Transferred {amount} to {to_user}"
        )  # <-- Уязвимость! CWE-352
...
def add_to_global_cache(request):
    ...
    GLOBAL_CACHE[key] = (
        value  # <-- Уязвимость! Аналог "утечки памяти" (CWE-404, CWE-772)
    )
